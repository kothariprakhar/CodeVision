...
function computeLayout(nodes, edges) {
  ...
  const iterations = nodes.length > 180 ? 70 : (nodes.length > 120 ? 95 : 120);

  const simulation = forceSimulation(layoutNodes)
    .force('charge', forceManyBody().strength(nodes.length > 120 ? -24 : -30))
    .force('center', forceCenter(0, 0, 0))
    .force('link', forceLink(layoutLinks).id(node => node.id).distance(nodes.length > 120 ? 3.4 : 3.8).strength(0.18))
    .stop();
...
function GraphScene({ graphData, selectedNodeId, onSelectNode, hotspotNodeIds }) {
  const layout = useMemo(
    () => computeLayout(graphData.nodes, graphData.edges),
    [graphData.nodes, graphData.edges]
  );
  const [hoveredNodeId, setHoveredNodeId] = useState(null);
  const hotspotSet = useMemo(() => new Set(hotspotNodeIds || []), [hotspotNodeIds]);
  ...
  return (
    <>
      ...
      {graphData.nodes.map(node => {
        ...
        const isHotspot = hotspotSet.has(node.cluster_id);
        ...
        const emissiveIntensity = isSelected
          ? 0.58
          : (isHotspot ? 0.42 : (0.2 + (node.hotness_score || 0) * 0.32));
...
class GraphErrorBoundary extends Component {
  ...
  componentDidCatch(error) {
    if (this.props.onError) {
      this.props.onError(error instanceof Error ? error.message : '3D rendering error');
    }
  }
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="h-[560px] w-full rounded-xl border border-yellow-500/30 bg-yellow-500/10 p-4 text-sm text-yellow-200">
          3D renderer failed. Falling back to 2D overview.
        </div>
      );
    }
...
export default function ModuleGraph3DView({
  graphData,
  selectedNodeId,
  onSelectNode,
  onRenderError,
  ...
  return (
    <GraphErrorBoundary onError={onRenderError}>
      <div className="h-[560px] w-full rounded-xl border border-white/10 bg-black/30">
        <Canvas
          camera={{ position: [10, 8, 14], fov: 52 }}
          dpr={[1, 1.5]}
          onCreated={({ gl }) => {
            const canvas = gl.domElement;
            canvas.addEventListener('webglcontextlost', event => {
              event.preventDefault();
              if (onRenderError) onRenderError('WebGL context lost');
            }, { once: true });
          }}
        >
          <GraphScene
            ...
            hotspotNodeIds={graphData.hotspotNodeIds}
          />
        </Canvas>
      </div>
    </GraphErrorBoundary>
