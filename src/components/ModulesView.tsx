...
// ABOUTME: Module grid view showing architecture nodes with type icons and connection highlighting.
// ABOUTME: Clicking a card shows a detail panel with connections, files, and description.
...
export default function ModulesView({ architecture }: ModulesViewProps) {
  ...
  const connections = useMemo(() => {
    ...
    type Connection = { node: ArchitectureNode; edgeType: string };

    const outgoing: Connection[] = edges
      .filter((e) => e.from === selectedNodeId)
      .flatMap((e) => {
        const node = nodes.find((n) => n.id === e.to);
        return node ? [{ node, edgeType: e.type }] : [];
      });
  ...
  const connectedNodeIds = useMemo(() => {
    ...
    connections.outgoing.forEach((c) => ids.add(c.node.id));
    connections.incoming.forEach((c) => ids.add(c.node.id));
    return ids;
  }, [connections]);

  if (!nodes.length) {
    return <div className="text-center text-gray-500 py-8">No architecture data available</div>;
  }

  return (
    <div>
      <div className="text-xs text-gray-500 mb-4">
        {nodes.length} modules · {edges.length} connections · Click a module to explore connections
      </div>
      <div className="flex gap-5">
        ...
        <div className="grid grid-cols-3 gap-3 flex-1">
          {nodes.map((node) => {
            ...
            const isOutgoing = !isSelected && connectedNodeIds.has(node.id) && connections.outgoing.some((c) => c.node.id === node.id);
            const isIncoming = !isSelected && connectedNodeIds.has(node.id) && connections.incoming.some((c) => c.node.id === node.id);
            const isDimmed = selectedNodeId !== null && !isSelected && !connectedNodeIds.has(node.id);
            ...
            if (isSelected) {
              ...
              cardStyle = { backgroundColor: config.selectedBg };
            } else if (isOutgoing) {
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start gap-2 mb-2">
                  ...
                  {isSelected && <span className={`flex-shrink-0 w-2 h-2 rounded-full mt-1 ${config.dotClass}`} aria-label="selected" />}
                  {!isSelected && isOutgoing && <span className="flex-shrink-0 text-[10px] font-bold text-yellow-400 mt-0.5 leading-none">→</span>}
                  {!isSelected && isIncoming && <span className="flex-shrink-0 text-[10px] font-bold text-blue-400 mt-0.5 leading-none">←</span>}
                </div>
                {/* File count */}
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-sm text-gray-400">{node.files.length} {node.files.length === 1 ? 'file' : 'files'}</span>
                </div>
              </button>
            );
          })}
            ...
            {selectedNode ? (
              <div className="space-y-3">
                <div>
                  ...
                  <span className={`text-xs ${TYPE_CONFIG[selectedNode.type].textClass}`}>{TYPE_CONFIG[selectedNode.type].label}</span>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-0.5 max-h-16 overflow-y-auto">
                    {selectedNode.files.map((f, i) => <div key={i} className="text-xs text-gray-400 truncate">{f}</div>)}
                  </div>
                </div>
              ...
              <div className="flex items-center justify-center h-full text-gray-600 text-xs pt-16">Select a module to explore</div>
            )}
          </div>
        </div>
      </div>
